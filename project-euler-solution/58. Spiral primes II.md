...
### Accuracy 
Given a composite number, the test has a more than 75% chance to declare n as a composite, which is at most 25% (=![eq](https://latex.codecogs.com/gif.latex?4^{-1})) chance to declare it as a prime. That sounds bad, doesn't it? In fact no. Because we can pick another 1 <= a < n and re-run the test. Since two tests are independent, there is at most 6.25% (=![eq](https://latex.codecogs.com/gif.latex?4^{-2})) chance to declare the number as a prime. We can continue doing this until there is little chance of declaring it as a prime. 
